...
PRO aycamp_long_reduce_work, filename, scifile, ISLIT = ISLIT $
                      ...
                      , NSIGMA_CR = NSIGMA_CR1, PSF_CR_FUDGE = PSF_CR_FUDGE1, novac=novac, niter=niter
...
;;   maxobj = 1
   sigrej = 25.0
; jm10jun25ucsd - the CR-rejection is much better now so allow cr-zapping!
...
ENDIF ELSE BEGIN
   ...
   IF n_elements(skytrace1) THEN SKYTRACE = SKYTRACE1
...
IF KEYWORD_SET(SKYTRACE) THEN BEGIN
    ...
    pixset = long_wavepix(sciimg, tset_slits, fwhm = fwhmset.MEDIAN $
                          ...
                          , piximg_in = piximg, ONLY_SLITS = ISLIT)
    piximg = long_wpix2image(pixset, tset_slits, XFIT = xfit $
                             , waveimg = waveimg)
ENDIF
...
skyimaget = long_skysub(sciimg, sciivar, piximg, slitmask, skymask1, edgmask $
                        , bsp = bsp, ISLIT = ISLIT)
...
IF NOT KEYWORD_SET(NOZAP) THEN BEGIN
    ...
    aycamp_la_cosmic, sciimg, outlist=cleanimg, masklist=crmask, $
      /zeroindex, sigclip=sigclip, objlim=2.0, sigfrac=sigfrac
    sciivar = sciivar*(crmask EQ 0)
;; old code commented out - jm10jun17ucsd
...
if (keyword_set(novac) eq 0) then begin ; jm10jun16ucsd
...
IF KEYWORD_SET(SKYFILE) and not keyword_set(NOFLEX) THEN begin 
    ...
    long_flexure, tfinal_struct, skyfile, flg_skyfile, QAFILE = QAFILE, MAXFLEX=maxflex, $
                  MAXGOOD=MAXGOOD
 ENDIF
