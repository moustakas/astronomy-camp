...
;-  
;-----------------------------------------------------------------------------
PRO aycamp_long_reduce, planfile, clobber = clobber, verbose = verbose $
                 ...
                 , BIN_RATIO=bin_ratio, ARC_INTER=arc_inter, niter=niter, $
  justcalib=justcalib, juststd=juststd, justsci=justsci, calibclobber=calibclobber
...
if (NOT keyword_set(planfile)) then planfile = findfile('plan*.par')
   ...
   if (n_elements(planfile) GT 1) then begin
      for i=0L, n_elements(planfile)-1L do begin
          ...
          splog, '------------------------'
          aycamp_long_reduce, planfile[i], clobber = clobber, verbose = verbose $
                       , NOFLEX = noflex, NOZAP = NOZAP, NOHELIO = NOHELIO $
                       , ONLYSCI = ONLYSCI, HAND_X = HAND_X, HAND_Y = HAND_Y $
                       , HAND_FWHM = HAND_FWHM, LINELIST = linelist $
                       , USAGE = USAGE, FILESTD = FILESTD1, CHK = CHK $
...
;          iflat = where(qtwi OR qdome)
         ...
         if (narc GT 0) then begin
             iarc = jndx[ii]
             for jarc = 0, narc-1 do begin ; jm10jun19ucsd - reduce all the arcs!
           ...
           (not strmatch(planstr[0].instrument,'GMOS-SX')) THEN BEGIN 
             ...
             if (pixct EQ 0 OR illumct EQ 0 OR keyword_set(calibclobber)) then begin
                 ...
                 splog, 'Generating pixel flat for INSTRUMENT=' $
                        ...
                        , mask_list[imask]
                 if keyword_set(clobber) then begin ; jm09nov06ucsd
                    pixct = 0
                    illumct = 0
                 endif
...
;splog, 'NOTE!!!!!!'
;if strtrim(planstr[j].flavor,2) eq 'science' then clobber = 1 else clobber = 0
;nolocal = 1
;prof_nsigma = 20.0
...
; choose the wavelength map
                 ...
                 aycamp_long_reduce_work, djs_filepath(planstr[j].filename $
                                   ...
                                   , NOHELIO = NOHELIO $
                   , novac=novac, niter=niter $ ; jm10jun16ucsd
