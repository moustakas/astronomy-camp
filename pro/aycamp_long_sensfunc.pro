...
;------------------------------------------------------------------------------
FUNCTION aycamp_long_sensfunc, scifile, sensfuncfile $
                        , std_name = std_name $
...
; are SCIFILE and STD_NAME vectors?  jm09dec19ucsd
  ...
  if (nstd gt 1) then begin
     for ii = 0, nstd-1 do begin
        sens1 = aycamp_long_sensfunc(scifile[ii],sensfuncfile,$
...
;    for ii = 0, nstd-1 do flux[*,ii] = flux[*,ii]
...
; make a qaplot     
     ...
     djs_plot, wave, sensfunc, ps=6, sym=0.2, xsty=3, ysty=3, position=pos, $
       xtitle='Wavelength (\AA)', $ ; yrange=[max(sensfit),min(sensfit)], $
       ytitle='SF [(electron s^{-1}) / (erg s^{-1} cm^{-2} \AA^{-1})]', $
       yrange=[80,im_min(sensfunc,sigrej=2.5)]
