...
; pull out the right zeropoint
    zptfile = getenv('AYCAMP_DATA')+'2010/kp09m/'+$
...
pro kp09m_2010_unpack, struct, indir=indir, outdir=outdir, $
...
pro reduce_kp09m_2010, night=night, inspect=inspect, mkbias=mkbias, $
  ...
  solve=solve, unpack_projects=unpack_projects, clobber=clobber
    ...
    datapath = getenv('AYCAMP_DATA')+'2010/kp09m/'
...
; throw out crap objects
          case night[inight] of
             '16jun10': begin
                keep = where($
                  (strmatch(allstruct.img_root,'*16jun2010.00[2-3]?.fits*') eq 0) and $ ; crap zeros
                  (strmatch(allstruct.img_root,'*16jun2010.0040.fits*') eq 0) and $     ; crap zeros

                  (strmatch(allstruct.img_root,'*16jun2010.00[0-1]?.fits*') eq 0) and $ ; crap zeros
                  (strmatch(allstruct.img_root,'*16jun2010.0020.fits*') eq 0) and $     ; crap zeros
                  (strmatch(allstruct.img_root,'*16jun2010.0055.fits*') eq 0) and $     ; U-band standard star

                  (strmatch(allstruct.img_root,'*16jun2010.007[0-3].fits*') eq 0))      ; service
; fix some headers
             ...
             '17jun10': begin
                keep = where($
                  (strmatch(allstruct.img_root,'*17jun2010.0033.fits*') eq 0) and $ ; saturated!
                  (strmatch(allstruct.img_root,'*17jun2010.0034.fits*') eq 0))
             end
             '18jun10': begin
                keep = where($
                  (strmatch(allstruct.img_root,'*18jun2010.0142.fits*') eq 0))
             end
