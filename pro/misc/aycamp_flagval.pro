...
;
; CALLING SEQUENCE:
;   value = aycamp_flagval(flagprefix, label)
;
; INPUTS:
...
;
; COMMENTS:
;   This function is the inverse of AYCAMP_FLAGNAME().
;
; PROCEDURES CALLED:
;   splog
;   yanny_free
...
;
; DATA FILES:
;   $IMPRO_DIR/imaging_redux/aycamp_maskbits.par
;
; REVISION HISTORY:
;   02-Apr-2002 Written by D. Schlegel, Princeton.
;   29-Jul-2008 imported into IMPRO, Moustakas
;   30-May-2010 Adopted for Astronomy Camp by J. Moustakas
;-

function aycamp_flagval, flagprefix, inlabel, check=check

; Declare a common block so that the mask names are remembered between calls.

;   common com_aycamp_maskbits, maskbits

    if (n_params() NE 2 OR n_elements(flagprefix) NE 1) then begin
       print, 'Syntax - value = aycamp_flagval(flagprefix, label)'
       return, ''
    endif
...
; (After that, store this info in a common block.)
    ...
    maskfile = filepath('aycamp_maskbits.par',root_dir=getenv('AYCAMP_DIR'),$
      subdirectory='imaging_redux')
...
;      maskfile = sg1120_path(/sex)+'aycamp_maskbits.par'
