...
;    used  to run most of the programs in the AYCAMP imaging reduction
;    package.
;
; INPUTS:
;
...
;
; EXAMPLES:
;   aycamp_img_strct, nght1_strct, /MKDIR
;
;
...
pro aycamp_img_strct, struct, ccd=ccd, tel=tel, LIST=list, $
  NOMKDIR=nomkdir, NOFILE=nofile, OUTFIL=outfil, IMG=img, $
  clobber=clobber

;  Optional Keywords
    if n_elements(CCD) eq 0 then ccd = 'S2KB'
    if n_elements(OUTFIL) eq 0 then outfil = 'imgstrct.fits'
    if n_elements(TEL) eq 0 then tel = 'kp09m'
    ...
    if not keyword_set( IMG ) then begin
       if not keyword_set( LIST ) then begin
          ...
          if nimg EQ 0 then begin
             print, 'aycamp_img_strct: No images in Raw!'
             stop
             return
          endif
       endif else begin
          if x_chkfil(list[0]) NE 1 then begin
             print, 'aycamp_img_strct: Trouble with filename ', list
             return
          endif
...
;  Parse the Header
       ...
       if keyword_set(racrd) then struct[q].RA = 15D*im_hms2dec(strtrim(sxpar(head,racrd),2))
       if keyword_set(deccrd) then struct[q].DEC = im_hms2dec(strtrim(sxpar(head,deccrd),2))
...
; parse miscellaneous header quantities
       case ccd of
          'S2KB': begin
; pixel scale and frame number
...
;    stetfile = getenv('AYCAMP_DIR')+'/data/phstd_stetson.tfits'
;    if (file_test(stetfile) eq 0) then begin
;       splog, 'Stetson photometric catalog not found! Check that ${AYCAMP_DIR} is set!'
;    endif else begin
